...
def run_RH(data, F, L, T, write_csv, opt_gap, verbose, solver_seed = None, benchmark=False):
    ...
    for i, (window, fix_periods) in enumerate(zip(windows, fixes)):
        ...
        result = build_RH_subprobs_t(data, window, init_states if i>0 else {}, fix_periods, 
                                     print_carryover = prt_cry, opt_gap = opt_gap, warm_start = warm_start, solver_seed = solver_seed, benchmark=benchmark)
...
def sweep_RH(data, T = 36, F_vals = range(1,37), L_vals = range(1,37), seeds=(1,2), opt_gap = opt_gap, only_valid = False, csv_path = "rh_grid_results.csv", verbose = False):
            ...
            for s in seeds:
                try:
                    rh_time, ofv, _ = run_RH(
                        ...
                        write_csv=False,
                        solver_seed=s,
                        verbose=verbose and (s == seeds[0])  # print once per (F,L) if you want
                    )
...
#df = sweep_RH( data, T = 36, F_vals = range(1,37), L_vals = range(1,37), seeds = (1, 2), opt_gap = 0.05, only_valid = True, csv_path = "rh_T36_seed1.csv", verbose = True)
