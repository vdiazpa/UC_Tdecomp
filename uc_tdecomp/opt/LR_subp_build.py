...
def build_LR_subprobs(data, s_e, index_set):
    ...
    def ofv_start(m):
        ...
        return  start_cost  + on_cost + pow_cost + shed_cost + renew_cost + disch_cost + ch_cost + pnlty_on + pnlty_gen + pnlty_UObl + pnlty_DObl + pnlty_soc + 5000 * sum(m.SoC_Under[b] for b in m.StorageUnits)
    ...
    def ofv(m):
        ...
        return st_cost + shed_cost + on_cost + renew_cost + disch_cost + ch_cost + on_cpy_cost + pow_cost + pow_cpy_cost + penalty_costs + 5000 * sum(m.SoC_Under[b] for b in m.StorageUnits) 
    ...
    def ofv_end(m):
        ...
        return   st_cost + on_cost + pow_cost + shed_cost + renew_cost + disch_cost + ch_cost - 1 * (pnlty_on_cpy + pnlty_pow_cpy + pnlty_UObl_cpy + pnlty_DObl_cpy + pnlty_soc_cpy) + 5000 * sum(m.SoC_Under[b] for b in m.StorageUnits) 
